...
clustering_calls <- calls_scaled
clustering_calls$y <- NULL

#display clustering data
str(clustering_calls)
summary(clustering_calls)
```


```{r,cache=TRUE}
# sample data (due to storage issue) to select optimal number of clusters d
sample_size <- .4*nrow(clustering_calls)
sample_rows <- sample(1:nrow(clustering_calls), sample_size)
sample_calls <- clustering_calls[sample_rows, ]

#fviz_nbclust(sample_calls[,], kmeans, method = "wss")
#fviz_nbclust(sample_calls[,], kmeans, method = "gap_stat")
#fviz_nbclust(sample_calls[,], kmeans, method = "silhouette")
```
## Cluster making
...
# making the clusters
optimal_k <- 5 # change this later
km <- kmeans(clustering_calls, optimal_k)

#visualize clusters
fviz_cluster(km, data = clustering_calls)

#adding clusters to models
calls_scaled$cluster <- as.factor(km$cluster)
```
...
```{r}
#create new data fram with clusters
calls_dummy2 <- as.data.frame(model.matrix(~. -1, data = calls_scaled))
